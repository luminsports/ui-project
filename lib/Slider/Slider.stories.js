  ...
  template: `
    <Slider class="fixed" :step="5" :min="0" :max="220">
      <SliderRail class="bg-gray-500" >
        <SliderThumb v-model="T1" :prevent-overlap="10" v-slot='{ pos, value }'>
          <div :style='pos' class='h-5 w-5 bg-blue-500' />
        </SliderThumb>
        <SliderThumb v-model="T2" :disabled="false" v-slot='{ value, index, disabled, hovered, pressed, pos }'>
          <div :style='pos' class='h-5 w-5 bg-red-500' />
        </SliderThumb>
        <SliderThumb v-model="T3" v-slot='{ pos, value }'>
          <div :style='pos' class='h-5 w-5 bg-green-500' />
        </SliderThumb>
      ...
      <SliderLabels :at='[0, 25, 30, 75, 100]' v-slot='{ pos, value, index, interpolatedValue }'>
        <div :style='pos' class='h-5 w-1 bg-gray-200/40'>{{ value }}</div>
      </SliderLabels>
    </Slider>`,
})
